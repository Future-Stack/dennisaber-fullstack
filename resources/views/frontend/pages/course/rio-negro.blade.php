...
<div class="portal-notice"><strong>Technisches Kursportal</strong><span>Keine Bestellung auf dieser Website. <a href="/rausgehen-reicht-nicht/abenteuer">Kurse im Verkaufsportal ansehen →</a></span></div><header class="portal-header course-header"><a href="{{ route('home') }}" class="portal-brand"><strong>DENNIS BESSELER</strong><span>Kursportal</span></a><nav><a href="{{ route('home') }}">Alle Kurse</a><a href="{{ route('copy-protection') }}">Kopierschutz</a><a href="{{ route('payment') }}">Zahlung</a><a href="{{ route('faster-processing') }}">Schnellere Bearbeitung</a><a href="#anmelden">Anmelden</a></nav></header><section class="course-hero"><div class="course-hero-copy"><p class="eyebrow">Wahres Audio-Abenteuer für Erwachsene</p><h1>Erst erleben. Dann entscheiden.</h1><p class="course-lead">Hören Sie die erste Etappe, blättern Sie im Expeditionsbuch und entscheiden Sie selbst, wie Sie aufbrechen würden.</p><div class="course-price"><strong>24,90 €</strong><span>einmalig · kein Abo</span></div><div class="access-badge"><b>Persönlicher Kurszugang</b><span>30 Tage ab Freischaltung</span><small>Kein Abonnement · keine automatische Verlängerung</small></div><a class="order-cta" href="/rausgehen-reicht-nicht/abenteuer">Kurs im Verkaufsportal ansehen <span>→</span></a><a href="{{ route('faster-processing') }}" class="service-cta"><span>Bereits bestellt oder überwiesen?</span><strong>Schnellere Bearbeitung →</strong></a></div><aside class="course-login" id="anmelden"><p class="login-label">Bereits freigeschaltet?</p><h2>Persönlichen Kurs öffnen</h2><form method="POST" action="{{ route('login.store') }}" autocomplete="off">
                @csrf
                <label>
